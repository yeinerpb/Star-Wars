...
function Characters() {
  ...
  const [allFilms, setAllFilms] = useState([]); // nuevo estado para guardar todas las películas
  ...
  useEffect(() => {
    ...
    fetchFilms();
  }, []); // nuevo efecto para obtener todas las películas
          ...
          <div>
            ...
            <Films
              films={allFilms.filter((film) =>
                selectedCharacter.films.includes(film.url)
              )} // filtrar solo las películas en las que participa el personaje
            />
          </div>
        )}
